...
//						"PYTHONHOME=C:\\OSGeo4W\\apps\\Python25",
	...
	private void executeGrassModuleStarter() {
		...
		try {
			...
			if(!errors.equals("")){
                String logBaseDir = WebProcessingService.getApplicationBaseDir() + File.separator + LOGS_DIR_NAME;
				File baseDirFile = new File(logBaseDir);
				...
				File serverLog = new File(logBaseDir + fileSeparator + uuid + logFilename);
                ...
                String message = String.format("An error occured while executing the GRASS GIS process. See the log under %s for more details.",
                                               WPSConfig.getInstance().getServiceBaseUrl() + "/" +
                                               LOGS_DIR_NAME + "/" + uuid + logFilename);
